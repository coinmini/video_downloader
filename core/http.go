...
func (h *HttpServer) batchExportExcel(w http.ResponseWriter, r *http.Request) {
	...
	headers := []string{"序号", "描述", "点赞", "投币", "收藏", "转发", "评论", "播放量", "链接"}
	colWidths := []float64{8, 60, 12, 12, 12, 12, 12, 12, 80}
	...
	for idx, item := range data.Items {
		...
		setIntOrStr(f, sheet, 4, row, item.CoinCount)
		setIntOrStr(f, sheet, 5, row, item.FavCount)
		setIntOrStr(f, sheet, 6, row, item.ForwardCount)
		setIntOrStr(f, sheet, 7, row, item.CommentCount)
		setIntOrStr(f, sheet, 8, row, item.ViewCount)
		f.SetCellValue(sheet, cellName(9, row), item.Url)
	}
